...
// DTCG is consumable via the ./tokens/* subpath exports instead).
const categoryVarName = (fmt, category) => `${camel(category)}${fmt.infix}Global`
const categoryImportLine = (fmt, category) => {
  const v = categoryVarName(fmt, category)
  if (fmt.key === 'json') return `const ${v} = require('./json/${category}/global.json')`
  return `import * as ${v} from './${fmt.key}/${category}/global.${fmt.ext}'`
}
...
for (const fmt of FORMATS) {
  ...
  if (fmt.key !== 'colors')
    for (const category of CATEGORIES) out += `${categoryImportLine(fmt, category)}\n`
  out += '\n'
}
...
const formatTree = (fmt) => {
  ...
  const categories =
    fmt.key === 'colors'
      ? []
      : CATEGORIES.map((c) => `    ${camel(c)}: { global: ${categoryVarName(fmt, c)} },`)
  return [
    `  ${fmt.key}: {`,
