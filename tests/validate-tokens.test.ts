...
describe('validate-tokens', () => {
  ...
  const failures: Array<[fixture: string, diagnostic: string]> = [
    ...
    // The formats render an object colour via build/formats.mjs `colorFunction`, which
    // supports srgb/hsl/oklch only. Style Dictionary 5 turns its throw into a warning plus
    // a fallback, so an unsupported (or absent) space would otherwise reach the built CSS
    // as "[object Object]" — catch it at the source instead.
